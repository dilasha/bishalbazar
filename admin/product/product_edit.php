...
if (isset($_POST['btnProdEdit'])) {
	...
	$query = "UPDATE PRODUCT SET prodName=:editProdName, prodCategory=:editProdCat, shopID=:editProdShop, 
	prodRate=:editProdPrice, prodFeat=:editProdFeat, prodImg=:editProdImg where prodID='$prodID'";
	$parse = oci_parse($connection, $query);
				...
				<form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select required  name="ddProdShop" class="form-control input-sm">
								...
								<?php
								$uID = $_SESSION['userid'];
								$query = "SELECT * FROM shop where shopStatus='Verified'";
								$parse = oci_parse($connection, $query);
								oci_execute($parse);
								while ($row = oci_fetch_assoc($parse)) {
									$sel = "";
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							...
							<img src="http://localhost/Team6/images/products/<?php echo $eProdImg ?>"/>
						</div>
					</div>
					<div class="form-group">
